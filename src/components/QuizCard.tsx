...
const QuizCard = ({ chapter }: Props) => {
  ...
  return (
    <div className="flex-[1] mt-16 ml-8">
      <h1 className="text-2xl font-bold">Make Sense?</h1>
      <div className="mt-2">
        {chapter.questions.map((question) => {
          ...
          return (
            <div
              key={question.id}
              className={cn("p-3 mt-4 border border-secondary rounded-2xl", {
                "bg-emerald-600": questionState[question.id] === true,
                "bg-rose-600": questionState[question.id] === false,
      ...
      <Button className="w-full mt-2 rounded-xl hover:bg-blue-600 dark:hover:bg-gray-400 bg-blue-400 dark:bg-gray-200 font-semibold" size="lg" onClick={checkAnswer}>
        Check Answer
